...
<script>
	...
	webSocket.onopen = function(event){
		chatWindow.innerHTML += "웹소켓 서버 연결되었습니다.<br/>";
	};
	
	// 웹소켓이 닫혔을때(서버와의 연결이 끊겼을 때) 실행
	...
	webSocket.onerror = function(event){
		chatWindow.innerHTML += "채팅중 에러가 발생하였습니다.";
	};
	
	// 메시지를 받았을 때 실행
	webSocket.onmeassage = function(event){
		var message = event.data.split("|"); // 대화명과 메시지 분리 (예 : aaa | hi)
		var sender = message[0]; // 보낸사람의 대화명 (예 : aaa)
		console.log("메시지 : " + message);
		console.log("작성자 : " + sender);
